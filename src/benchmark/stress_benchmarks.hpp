...
namespace benchmark {
	...
	public:
		...
		static void benchmarkHighContention(size_t threads, size_t ops_per_thread) {
			using PoolType = OptimizedObjectPool<LargeTestObject, PoolSize, true, std::pmr::polymorphic_allocator<LargeTestObject>, LocalCacheSize>;
			PoolType contention_pool;
			contention_pool.prewarm(PoolSize / 16);

			std::string name = "RawPool[P=" + std::to_string(PoolSize) + ",C=" + std::to_string(LocalCacheSize) + "] HC";
		...
		static void benchmarkMemoryPressure() {
			...
			{
				using PoolType = SharedOptimizedObjectPool<LargeTestObject, lockfree_config::DEFAULT_POOL_SIZE, true>;
				PoolType pool;
				pool.prewarm(512);
		...
		static void benchmarkThreadContentionAnalysis() {
			...
			for (auto threads : thread_counts) {
				...
				for (auto cache_size : cache_sizes) {
					switch (cache_size) {
						case 1: {
							using PoolType = OptimizedObjectPool<LargeTestObject, 1024, true, std::pmr::polymorphic_allocator<LargeTestObject>, 1>;
							PoolType pool;
							pool.prewarm(256);

							auto result = MultithreadedBenchmarks::benchmarkPoolMT("Cache=" + std::to_string(cache_size), threads, contention_ops, pool);
							auto stats = pool.get_stats();
						...
						case 4: {
							using PoolType = OptimizedObjectPool<LargeTestObject, 1024, true, std::pmr::polymorphic_allocator<LargeTestObject>, 4>;
							PoolType pool;
							pool.prewarm(256);
						...
						case 8: {
							using PoolType = OptimizedObjectPool<LargeTestObject, 1024, true, std::pmr::polymorphic_allocator<LargeTestObject>, 8>;
							PoolType pool;
							pool.prewarm(256);
						...
						case 16: {
							using PoolType = OptimizedObjectPool<LargeTestObject, 1024, true, std::pmr::polymorphic_allocator<LargeTestObject>, 16>;
							PoolType pool;
							pool.prewarm(256);
						...
						case 32: {
							using PoolType = OptimizedObjectPool<LargeTestObject, 1024, true, std::pmr::polymorphic_allocator<LargeTestObject>, 32>;
							PoolType pool;
							pool.prewarm(256);
